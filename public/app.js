...
function renderReceivableSummary(rows, month) {
  ...
  </div><div class="module-policy-note"><strong>회수조치 점검</strong><span>신규출고 보류 ${formatNumber(holdCount)}건 · 추가여신 대표님 승인 대상 ${formatNumber(approvalCount)}건 · 회수약속 미이행과 90일 이상 채권은 아침회의에 우선 표시됩니다.</span></div>`;
}
...
function renderReceivableRows(rows) {
  ...
  $("#recordTableBody").innerHTML = rows.length ? rows.map(row => {
    ...
    const approvals = [truthy(payload.shipment_hold) ? "출고보류" : "", truthy(payload.legal_review) ? "법무검토" : "", truthy(payload.representative_approval) || detail.maxOverdueDays >= 90 ? "대표님 승인" : ""].filter(Boolean).join(" · ");

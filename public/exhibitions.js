...
(function () {
  'use strict';
  let ctx, options, listing, detail, activeTab='basic', mode='list', editing=null;
  const filters={list:{year:String(new Date().getFullYear())},calendar:{}};
  ...
  const today=()=>listing?.today||new Date().toISOString().slice(0,10);
  ...
  function saveFilters(){try{sessionStorage.setItem('maps:exhibition-filters',JSON.stringify({filters,mode,calendarMonth}));}catch(_e){}}
  function init(context){ctx=context;try{const x=JSON.parse(sessionStorage.getItem('maps:exhibition-filters')||'null');if(x){Object.assign(filters.list,x.filters?.list);Object.assign(filters.calendar,x.filters?.calendar);mode=x.mode==='calendar'?'calendar':'list';calendarMonth=x.calendarMonth||calendarMonth;}}catch(_e){} }
  async function load(){
    if(!ctx)return;
  ...
  function select(name,values,value,placeholder='전체'){
    return `<select name="${esc(name)}" aria-label="${esc(name)}"><option value="">${esc(placeholder)}</option>${Object.entries(values).map(([k,v])=>`<option value="${esc(k)}" ${String(value)===String(k)?'selected':''}>${esc(v)}</option>`).join('')}</select>`;
  }
  function filterMarkup(){
    const f=filters[mode], countries=Object.fromEntries([...new Set(listing.items.map(e=>e.country))].sort().map(v=>[v,v]));
    ...
    if(mode==='list'){
      const years=[...new Set([String(new Date().getFullYear()),...listing.items.map(e=>e.start_date.slice(0,4)),f.year].filter(Boolean))].sort();
  ...
  function filtered(){const f=filters[mode];return listing.items.filter(e=>{
    if(mode==='list'&&f.year&&e.start_date.slice(0,4)!==f.year)return false;
    if(mode==='list'&&f.month&&e.start_date.slice(5,7)!==f.month)return false;
  ...
  function renderMain(){
    ...
    root().innerHTML=`<div class="ex-heading"><div><h2>전시회 / Exhibition</h2><p>행사 결정부터 준비, 현장운영, 귀국 후 Follow-up까지</p></div><div>${writable()?button('체크리스트 기준 관리','data-ex-templates')+button('+ 행사 등록','data-ex-new','primary'):badge('읽기 전용')}</div></div><div class="ex-kpis">${cards.map(([t,v])=>`<article><span>${t}</span><strong>${v}</strong></article>`).join('')}</div><div class="ex-toolbar"><div class="ex-segment">${button('List',`data-ex-mode="list" aria-pressed="${mode==='list'}"`,mode==='list'?'selected':'')}${button('Calendar',`data-ex-mode="calendar" aria-pressed="${mode==='calendar'}"`,mode==='calendar'?'selected':'')}</div><small>KPI는 전체 기준 · List / Calendar 필터는 각각 유지됩니다.</small>${button('새로고침','data-ex-reload')}</div>${filterMarkup()}<div id="exResults">${mode==='list'?listMarkup():calendarMarkup()}</div>`;
    bindMain();
  }
  function listMarkup(){
  ...
  function calendarMarkup(){
    ...
    const weeks=[];for(let ws=start;ws<=end;ws=shift(ws,7)){
      ...
      weeks.push(`<div class="ex-week" style="min-height:${Math.max(112,40+lanes.length*30)}px"><div class="ex-days">${Array.from({length:7},(_,i)=>{const day=shift(ws,i);return `<div class="${day.slice(0,7)!==calendarMonth?'muted':''} ${day===today()?'today':''}"><time datetime="${day}">${Number(day.slice(8))}</time></div>`;}).join('')}</div><div class="ex-bars">${bars.map(({e,from,to,lane})=>`<button type="button" class="ex-calendar-event status-${esc(e.participation)}" style="grid-column:${from+1} / ${to+2};grid-row:${lane+1}" data-ex-open="${e.id}" data-event-start="${e.start_date}" data-event-end="${e.end_date}" aria-label="${esc(e.name)} ${e.start_date}~${e.end_date}" title="${esc(e.name)} · ${e.start_date}~${e.end_date}">${e.start_date<ws?'‹ ':''}${esc(e.name)}${e.end_date>we?' ›':''}</button>`).join('')}</div></div>`);
    }
    return `<div class="ex-calendar-heading">${button('‹ 이전','data-ex-month="-1"')}<label><span class="sr-only">Calendar 월</span><input type="month" id="exCalendarMonth" aria-label="Calendar 월" value="${calendarMonth}"></label>${button('다음 ›','data-ex-month="1"')}<div class="ex-legend">${Object.entries(options.choices.participation).map(([k,v])=>badge(v,'status-'+k)).join('')}</div></div><div class="ex-calendar"><div class="ex-weekdays">${['일','월','화','수','목','금','토'].map(x=>`<span>${x}</span>`).join('')}</div>${weeks.join('')}</div>`;
  }
  function bindMain(){
    root().onclick=async ev=>{const el=ev.target.closest('button');if(!el)return;
      if(el.hasAttribute('data-ex-reload'))return load();
      if(el.dataset.exMode){mode=el.dataset.exMode;saveFilters();renderMain();}
      if(el.hasAttribute('data-ex-reset')){filters[mode]={};saveFilters();renderMain();}
      ...
      if(el.dataset.exMonth){const d=utc(calendarMonth+'-01');d.setUTCMonth(d.getUTCMonth()+Number(el.dataset.exMonth));calendarMonth=iso(d).slice(0,7);saveFilters();renderMain();}
    };
    root().onchange=ev=>{if(ev.target.closest('#exFilters')){filters[mode][ev.target.name]=ev.target.value;saveFilters();renderMain();}if(ev.target.id==='exCalendarMonth'&&/^\d{4}-\d{2}$/.test(ev.target.value)){calendarMonth=ev.target.value;saveFilters();renderMain();}};

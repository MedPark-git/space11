(function () {
  ...
  function renderDetailTab(){
    ...
    if(cm.tab==='activities'){
      const taskRows=(c.major_tasks||[]).map(task=>`<button type="button" class="customer-map-account" data-major-task-id="${escapeHtml(task.id)}"><strong>${escapeHtml(task.title)}</strong><span>${escapeHtml(String(task.final_rag||'green').toUpperCase())} · ${escapeHtml(task.owner_name||'담당자 미지정')} · ${formatDate(task.hard_deadline_date||task.current_target_date)}</span></button>`).join('')||'<div class="empty-state">내부 UUID로 연결된 미결 주요업무가 없습니다.</div>';
